...
export const createNewBuildingEmployee =
  (salaryId, data) => async (dispatch) => {
    try {
      const res = await axios({
        method: "POST",
        baseURL: process.env.REACT_APP_URL_API,
...
export const updateBuildingEmployee = (empId, data) => async (dispatch) => {
  try {
    const res = await axios({
      method: "PUT",
      baseURL: process.env.REACT_APP_URL_API,
      url: `buildingEmployees/${empId}`,
      headers: {
        Authorization: "Bearer " + localStorage.getItem("token"),
        "Content-Type": "application/json",
      },
