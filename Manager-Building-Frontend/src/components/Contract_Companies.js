...
import {
  getAllCompanyForRegistration,
  getTheRestArea,
  createContract,
  getCompaniesForRegistrationByName,
...
const ContractCompany = () => {
  ...
  const restAreaFromReducer = useSelector(
    (state) => state.rentedAreas.restArea
  );
  ...
  const registerContract = () => {
    ...
    if (!isRentedAreaEmpty) {
      if (
        parseFloat(rentArea) > restAreaFromReducer ||
        parseFloat(rentArea) < 0
      ) {
        rentedAreaTag.parentElement.classList.remove("empty");
              ...
              <div className="form-post__wrapper">
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>Tên công ty: {company?.name}</strong>{" "}
                  </p>
                </div>
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>
                      {floor?.name} có diện tích còn lại là:{" "}
                      {restAreaFromReducer}
                    </strong>{" "}
                  </p>
                </div>
                ...
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>Tên khu vực thuê:</strong>
                  </p>
                  <input
                    value={position}
                    onChange={(e) => {
