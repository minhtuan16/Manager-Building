...
import {
  getAllWork,
  getWorkByEmpId,
  getWorkByTitleAndEmpId,
  getWorkById,
  getWorkByTitle,
  createNewWork,
  updateWork,
  deleteWork,
} from "../redux/actions/work.js";
...
const Work = () => {
  ...
  const popUpEditForm = (index) => {
    ...
    document.getElementById("start-date").value = moment(
      workList[index].startDate,
      "DD-MM-YYYY",
      true
    ).format("YYYY-MM-DD");
    document.getElementById("due-date").value = moment(
      workList[index].dueDate,
      "DD-MM-YYYY",
      true
    ).format("YYYY-MM-DD");
    document.getElementById("assigner").value = workList[index].assigner;
    document.querySelector(".emp-id").textContent =
      workList[index].buildingEmployee.id +
      " - " +
      workList[index].buildingEmployee.name;
  };
  ...
  const addOrEdit = () => {
    ...
    const emp_id = document
      .querySelector(".emp-id")
      .textContent.match(/\d+/)[0];

    // const emp_id = document.getElementById("emp-id").textContent;
    console.log(emp_id);

    // const employee = empData.find((employee) => employee.id === Number(emp_id));
    ...
    if (isAdd) dispatch(createNewWork(emp_id, data));
    else dispatch(updateWork(editIndex, emp_id, data));
  ...
  return (
    <>
      ...
      <div style={{ position: "relative", backgroundColor: "white" }}>
        ...
        <div
          style={{ display: isShow && !showEmpPopUp ? "block" : "none" }}
          className="modal"
        >
        ...
        <div
          style={{ display: showEmpPopUp ? "block" : "none" }}
          className="modal"
        >
          <div className="modal_overlay"></div>
          ...
          <div className="admin-post__wrapper">
            <div className="admin-post__head">
              ...
              <form action="" class="search-bar">
                ...
                />
                <button
                  onClick={() => searchWorkByTitleAndEmp()}
                  class="search-btn"
                  type="submit"
                >
                  <span>Search</span>
