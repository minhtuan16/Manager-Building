...
import { getSalaryById, getAllSalary } from "../redux/actions/salary";
import {
  createNewBuildingEmployee,
  deleteBuildingEmployee,
  getAllBuildingEmployees,
  getBuildingEmployeeById,
  updateBuildingEmployee,
  getBuildingEmployeeByName,
} from "../redux/actions/building_employee";
...
const BuildingEmployee = () => {
  ...
  useEffect(() => {
    setSalary(salaryData);
    setTimeout(() => {
      console.log("salary updated: ", salary);
    }, 600);
  }, [salaryData]);
  ...
  const popUpEditForm = (index) => {
    ...
    document.getElementById("address").value = buildingEmployees[index].address;
    // setDob(moment(buildingEmployees[index].dateOfBirth).format("YYYY-MM-DD"));
    document.getElementById("dob").value = moment(
      buildingEmployees[index].dateOfBirth,
      "DD-MM-YYYY",
      true
    ).format("YYYY-MM-DD");
    // document.getElementById('dob').value = buildingEmployees[index].dateOfBirth;
    document.getElementById("phone-no").value =
      buildingEmployees[index].phoneNo;
    document.getElementById("position").value =
      buildingEmployees[index].position;
    document.querySelector(".salary__id").textContent =
      buildingEmployees[index].salary.id;
    // document.querySelector('salary__id').textContent = "2";
    setIsShow(true);
  ...
  const editBuildingEmployee = () => {
    ...
    const data1 = {
      ...
      phoneNo: phoneNo,
      position: position,
      salary: { ...salaries[salaryId - 1] },
    };

    // console.log("data:-======= ",data1)
    dispatch(updateBuildingEmployee(buildingEmployees[editIndex].id, data1));
    setIconLoad(true);
    setTimeout(() => {
      dispatch(getAllBuildingEmployees());
      setIconLoad(false);
  ...
  const addBuildingEmployee = () => {
    ...
    const data = {
      address: address,
      name: name,
      dateOfBirth: new Date(moment(dob).format("YYYY-MM-DD")),
      phoneNo: phoneNo,
      position: position,
      salary: { ...salaries[salaryId - 1] },
    };
  ...
  const chooseSalary = () => {
    ...
    dispatch(getAllSalary());
    setTimeout(() => {
      console.log("this was ran: ", salariesData);
    }, 800);
  };
  ...
  const salaryChosen = (item) => {
    ...
    switch (item.salaryLevel) {
      case 1:
        document.querySelector(".position__id").textContent =
          "Thực tập sinh " + item.service.name;
        break;
      case 2:
        document.querySelector(".position__id").textContent =
          "Nhân viên " + item.service.name;
        break;
      case 3:
        document.querySelector(".position__id").textContent =
          "Trưởng bộ phận " + item.service.name;
        break;
    }
  };
    ...
    <>
      ...
      <div style={{ position: "relative", backgroundColor: "white" }}>
        <div
          ...
          style={{ display: iconLoad ? "block" : "none" }}
        >
          <div class="loader"></div>
        </div>
        <div
          style={{ display: isShow && !showSalaryPopUp ? "block" : "none" }}
          className="modal"
        >
          <div className="modal_overlay"></div>
          <div className="form-post">
            <div className="form-post__title dialog__title"></div>
            <div className="form-post__content">
              <div className="form-post__wrapper">
                <div className="form-post__field">
                ...
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>ID mức lương</strong>
                  </p>
                  <div style={{ display: "flex" }}>
                    <div
                      className="salary__id"
                      id="salary-id"
                      style={{
                        display: "block",
                        width: "100%",
        ...
        <div
          style={{ display: showSalaryPopUp ? "block" : "none" }}
          className="modal"
        >
          <div className="modal_overlay"></div>
          <div className="form-post active">
          ...
          <div className="admin-post__wrapper">
            <div className="admin-post__head">
              ...
              <form action="javascript:" class="search-bar">
                ...
                />
                <button
                  onClick={() => searchBuildingEmployeeByName()}
                  class="search-btn"
                  type="submit"
                >
                  <span>Search</span>
            ...
            <div
              style={{ fontSize: "20px", marginLeft: "-20px" }}
              className="admin-post__head"
            >
              Thông tin nhân viên tòa nhà
              <br />
            </div>
            <div
              className="admin-post__body"
              ...
              <table id="admin-post__table" className="table">
                <thead className="thead-dark">
                  <tr>
                    ...
                    <th style={{ width: "200px" }}>Vị trí</th>
                    <th style={{ width: "200px" }}>ID mức lương</th>
                    <th style={{ width: "200px" }}>Tiền lương</th>
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        {new Intl.NumberFormat("vi-VN", {
                          style: "currency",
                          currency: "VND",
                        }).format(item?.salary.salary)}
                      </td>
                      <td>Đang làm việc</td>
                      <td>
                        <button
                          onClick={() => popUpEditForm(index)}
