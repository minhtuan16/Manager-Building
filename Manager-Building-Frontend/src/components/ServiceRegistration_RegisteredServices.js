...
import {
  getAllRegisterdServices,
  deleteRegisterdService,
  updateRegisterdService,
} from "../redux/actions/registed_service";
...
function ServiceRegistration_RegisteredServices() {
  ...
  const popUpEditForm = (item) => {
    ...
    setStartDate(
      moment(item.startDate, "DD-MM-YYYY", true).format("YYYY-MM-DD")
    );
  ...
  const editRegisterdService = (item) => {
    if (item) {
      ...
      dispatch(
        updateRegisterdService(
          copyOfRegisteredService.id,
          copyOfRegisteredService
        )
      );
      setIconLoad(true);
      setTimeout(() => {
          ...
          <div className="form-post">
            ...
            <div className="form-post__content">
              <div className="form-post__wrapper">
                ...
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>Giá tiền tháng này:</strong>{" "}
                    {new Intl.NumberFormat("vi-VN", {
                      style: "currency",
                      currency: "VND",
                    }).format(registeredService?.currentPrice)}
                  </p>
                </div>
                <div className="form-post__field">
                  <p style={{ textAlign: "left" }}>
                    <strong>Ngày bắt đàu:</strong>
                  </p>
                  <input
                    value={startDate}
                    onChange={(e) => {
              ...
              <div className="form-post__control">
                ...
                <button
                  ...
                  onClick={() => editRegisterdService(registeredService)}
                >
          ...
          <div className="admin-post__wrapper">
            ...
            <div className="admin-post__body">
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        {item?.service?.required === 0 ? (
                          <button
                            onClick={() => cancelRegisteredService(item?.id)}
              ...
              <i style={{ fontSize: "15px" }}>
                (Chú ý: giá hiện tại tháng này của mỗi dịch vụ được tính bằng
                công thức: công ty dưới 10 người và thuê dưới 100 m2 thì có cùng
                mức giá dịch vụ cho mỗi dịch vụ , cứ thêm 5 người hoặc thêm 10
                m2 diện tích sàn thì đơn giá tăng lên 5% cho mỗi mục. Tiền dịch
                vụ được tính từ ngày đăng ký, hoặc từ đầu tháng đến thời điểm
                hiện tại. Số tiền dịch vụ được tính bằng tỉ lệ giữa ngày đã sử
                dụng trên tổng số ngày trong tháng)
              </i>
            </div>
          </div>
        </div>
